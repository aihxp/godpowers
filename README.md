...
Version 3.5.0 adds the work report: `npx godpowers report --since last` prints a
verification play-by-play from the evidence ledger, with an Attention section for
anything unverified, and advances a cursor so a fresh session sees only what is
new. It joins the 3.4.0 quarterback entry router (`npx godpowers route`) and the
close-on-evidence path from 3.1.0-3.3.0 (the evidence producer, `evidence.canClose`,
the enforced build and harden gates, and the `can-close` check the orchestrator
consults before closing). The current 3.x line keeps
...
```bash
npx godpowers mcp-info --project=.
npx -y -p godpowers@3.5.0 -p @godpowers/mcp@3.5.0 godpowers-mcp serve --project=.
```
...
```bash
npx -y -p godpowers@3.5.0 -p @godpowers/mcp@3.5.0 godpowers-mcp setup --host=codex --project=. --write
```
