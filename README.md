...
Version 3.8.0 adds a reusable lessons store: `npx godpowers lesson add|list`
keeps tagged, project- or global-scoped lessons, and a reflection that carries a
lesson auto-records one. It joins the 3.7.0 memory store (`npx godpowers
memory`), the 3.6.0 structured reflections (`npx godpowers reflect`), the 3.5.0
work report (`npx godpowers report`), the 3.4.0 quarterback entry router (`npx
godpowers route`), and the close-on-evidence path from 3.1.0-3.3.0 (the evidence
producer, `evidence.canClose`, the enforced build and harden gates, and the
`can-close` check the orchestrator consults before closing). The current 3.x line keeps
...
```bash
npx godpowers mcp-info --project=.
npx -y -p godpowers@3.8.0 -p @godpowers/mcp@3.8.0 godpowers-mcp serve --project=.
```
...
```bash
npx -y -p godpowers@3.8.0 -p @godpowers/mcp@3.8.0 godpowers-mcp setup --host=codex --project=. --write
```
