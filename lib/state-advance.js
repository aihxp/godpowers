...
function resultPass(projectRoot, target, previousStatus, status, updated, warnings) {
  return {
    command: 'state advance',
    verdict: 'pass',
    project: projectRoot,
    ...
    findings: [],
    summary: { updated: true, state: '.godpowers/state.json', views: [stateViews.PROGRESS_VIEW_PATH] }
  };
}
...
function advance(projectRoot, opts = {}) {
  ...
  try {
    ...
    return resultPass(projectRoot, freshTarget, previousStatus, request.status, updated, warnings);
  } finally {
