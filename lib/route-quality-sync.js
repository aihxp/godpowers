...
function detect(projectRoot) {
  ...
  for (const routePath of routes) {
    ...
    if (TIER_GATE_COMMANDS[command]) {
      ...
      if (actualGateCommand === TIER_GATE_COMMANDS[command]) {
        executableGateCount++;
      } else {
        addCheck(
          checks,
          `missing-executable-gate-${command.replace(/[^a-z0-9]+/gi, '-')}`,
          'stale',
          routePath,
          `${command} must declare standards.gate-command as ${TIER_GATE_COMMANDS[command]}.`,
          { spawn: 'god-auditor' }
        );
      }
    }
  ...
  addCheck(
    checks,
    'executable-tier-gate-policy',
    checks.some((check) => check.id.startsWith('missing-executable-gate-')) ? 'stale' : 'fresh',
    'routing/',
    `${executableGateCount} tier routes declare executable gate commands.`,
    { spawn: checks.some((check) => check.id.startsWith('missing-executable-gate-')) ? 'god-auditor' : null }
  );
  addCheck(
    checks,
    'agent-trace-policy',
