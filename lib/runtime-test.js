 ...
 *
 * Heuristic V1: looks for verbs like "click", "type", "navigate to",
 * "see", "expect". Returns null if no recognizable steps found.
 */
function parseFlow(text) {
  ...
  // Navigation
  const navMatch = text.match(/navigate\s+to\s+([\w\/\.\-:?=&]+)|visits?\s+([\w\/\.\-:?=&]+)|opens?\s+([\w\/\.\-:?=&]+)/i);
  if (navMatch) {
    steps.push({ kind: 'navigate', target: navMatch[1] || navMatch[2] || navMatch[3] });
  }
  // Click
  const clickMatch = text.match(/click(?:s|ed)?\s+(?:on\s+)?["']?([^."']+)["']?/i);
  if (clickMatch) {
    steps.push({ kind: 'click', target: clickMatch[1].trim() });
  }
  // Type
  const typeMatch = text.match(/type[s]?\s+(?:in\s+)?["']([^"']+)["']/i);
  if (typeMatch) {
    steps.push({ kind: 'type', text: typeMatch[1] });
  }
  // Expect / see
  const seeMatch = text.match(/(?:sees?|expects?|displays?|shows?)\s+["']?([^."']+)["']?/i);
  if (seeMatch) {
...
/**
 * Run a parsed flow on a Playwright/Vercel page.
