...
function detectMcp(projectRoot, opts = {}) {
  if (opts.mcp) return opts.mcp;
  const command = commandVersion('godpowers-mcp', ['--version'], { cwd: projectRoot });
  const sourcePackage = exists(path.join(projectRoot, 'packages', 'mcp', 'package.json'))
    && exists(path.join(projectRoot, 'packages', 'mcp', 'bin', 'godpowers-mcp.cjs'));
  return {
    available: Boolean(command || sourcePackage),
    source: command ? 'installed command' : (sourcePackage ? 'repo package source' : null),
    commandVersion: command || null
  };
}
...
function summary(report) {
  ...
  const mcp = report.guarantees && report.guarantees.mcp
    ? `; MCP ${report.guarantees.mcp.available ? `available via ${report.guarantees.mcp.source}` : 'not configured'}`
    : '; MCP unknown';
  if (report.level === 'full') return `full on ${report.host}${mcp}`;
  const gap = report.gaps && report.gaps.length > 0 ? `, ${report.gaps[0]}` : '';
  return `${report.level} on ${report.host}${gap}${mcp}`;
}
...
function render(report) {
  ...
  lines.push(`  MCP: ${report.guarantees.mcp && report.guarantees.mcp.available ? `available via ${report.guarantees.mcp.source}` : 'not configured'}`);
