...
because semantic-release reads the commit history on merge to main and
auto-computes the next version, generates release notes, prepends a
CHANGELOG entry, and publishes to npm.
...
- `chore(release): 0.15.0 [skip ci]` (auto-generated by semantic-release)

To skip release on an otherwise feat/fix commit, add `[skip release]`
to the message. To skip CI entirely on a release commit, add `[skip ci]`.
