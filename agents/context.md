...
- [DECISION] The package name is `godpowers`, and the current repository version is `2.5.0`.
...
- [OPEN QUESTION] Which external messy repository should become the first full host-run adoption case study after `2.5.0`? Owner: maintainer. Due: before the next broad product proof claim.
